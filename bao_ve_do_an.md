...
### 3.5. Kịch bản chạy thực tế theo từng Stage

Phần này dùng để trình bày khi thầy yêu cầu nhóm chạy hoặc giải thích quy trình. Mỗi Stage cần được thực hiện độc lập theo thứ tự: dừng firmware/listener cũ, khởi động broker, nạp đúng firmware, chạy listener đúng cấu hình, quan sát log và kiểm tra CSV.

#### Stage 1 – Baseline MQTT plaintext

**Mục đích:** tạo mốc đối chứng không có AES-GCM và không có TLS.

**Các bước thực hiện:**

1. Chọn thư mục `stage1_baseline`.
2. Chạy `flash_tool.ps1` để build và flash firmware Stage 1.
3. Khởi động Mosquitto với cổng MQTT thường `1883`.
4. Chạy listener với cấu hình Stage 1.
5. Listener subscribe topic `dothi/sensor/baseline`.
6. Chờ ESP32 đọc DHT11 và publish định kỳ khoảng 10 giây.
7. Kiểm tra log listener có nhiệt độ, độ ẩm, payload plaintext và trạng thái parse thành công.
8. Kiểm tra file `log_stage1.csv` có các cột `timestamp`, `msg_id`, `temp`, `humid`, `raw_payload_size`, `heap`, `enc_us`, `tls_ms`, `cpu_pct`.

**Kết quả cần giải thích:**

Stage 1 cho thấy pipeline cơ bản hoạt động đúng. Payload đọc được trực tiếp, không có thời gian AES và TLS. Kết quả mẫu có payload khoảng 94 B, heap khoảng 265 KB và CPU khoảng 0,2%.

**Câu nói khi bảo vệ:**

> Đây là baseline chứ không phải cấu hình bảo mật. Nhóm dùng Stage 1 để biết chi phí tài nguyên ban đầu trước khi thêm từng lớp bảo vệ.

#### Stage 2 – AES-256-GCM trên payload

**Mục đích:** kiểm tra bảo mật dữ liệu ở tầng ứng dụng, tách riêng khỏi chi phí TLS.

**Các bước thực hiện:**

1. Dừng firmware Stage 1 và listener cũ.
2. Chọn thư mục `stage2_aesgcm`.
3. Chạy `flash_tool.ps1` để nạp Stage 2.
4. Giữ broker MQTT ở cổng `1883`, vì Stage 2 chưa dùng TLS.
5. Chạy listener Stage 2 và subscribe topic AES-GCM tương ứng.
6. ESP32 tạo JSON cảm biến, mã hóa bằng AES-GCM, ghép nonce/ciphertext/tag rồi Base64.
7. Listener nhận chuỗi Base64, decode, tách nonce/ciphertext/tag và giải mã.
8. Kiểm tra authentication tag hợp lệ trước khi parse JSON.
9. Kiểm tra `log_stage2.csv` và trường `enc_us`.

**Kết quả cần giải thích:**

Payload tăng lên khoảng 168 B vì có nonce, tag và Base64. Thời gian AES trung bình khoảng 530,3 µs. TLS vẫn bằng 0 vì Stage 2 chỉ đo AES-GCM.

**Negative test:**

Nếu sửa một ký tự trong ciphertext hoặc Base64, tag sẽ không hợp lệ. Listener phải báo lỗi xác thực và không ghi dữ liệu giả vào CSV.

**Câu nói khi bảo vệ:**

> Base64 không phải cơ chế bảo mật. Cơ chế bảo mật là AES-GCM và authentication tag; Base64 chỉ giúp dữ liệu nhị phân được biểu diễn thành chuỗi để truyền qua MQTT.

#### Stage 3 – MQTT over TLS

**Mục đích:** đo chi phí bảo vệ kênh truyền và xác thực broker bằng TLS.

**Các bước thực hiện:**

1. Dừng Stage 2 và listener cũ.
2. Chọn thư mục `stage3_tls`.
3. Kiểm tra các file CA/certificate cần thiết và chạy `flash_tool.ps1`.
4. Khởi động Mosquitto với listener TLS ở cổng `8883`.
5. Chạy listener bằng URI `mqtts://` hoặc cấu hình TLS tương ứng.
6. Nạp CA certificate cho client.
7. Subscribe topic Stage 3.
8. Quan sát log `MQTT_EVENT_BEFORE_CONNECT` và `MQTT_EVENT_CONNECTED`.
9. Kiểm tra `tls_ms` và `log_stage3.csv`.

**Kết quả cần giải thích:**

Payload ứng dụng khoảng 96 B vì chưa mã hóa AES ở tầng payload. TLS handshake trung bình khoảng 755 ms. Heap giảm xuống khoảng 227,9 KB do TLS context, CA, certificate chain và buffer.

**Điểm cần nói chính xác:**

> `raw_payload_size` không phải toàn bộ kích thước TLS record. Nó chỉ là kích thước payload ứng dụng được listener xử lý. Overhead TLS được thể hiện rõ hơn qua thời gian handshake và mức giảm heap.

#### Stage 4 – AES-256-GCM kết hợp TLS

**Mục đích:** kiểm tra mô hình bảo mật nhiều lớp.

**Các bước thực hiện:**

1. Dừng Stage 3 và listener cũ.
2. Chọn thư mục `stage4_aesgcm_tls`.
3. Chạy `flash_tool.ps1` để build và flash Stage 4.
4. Khởi động Mosquitto TLS ở cổng `8883`.
5. Chạy listener với CA và khóa AES tương ứng.
6. ESP32 mã hóa payload bằng AES-GCM trước khi publish qua MQTTS.
7. Listener thiết lập TLS, nhận Base64, giải mã AES-GCM và kiểm tra tag.
8. Kiểm tra đồng thời `enc_us`, `tls_ms`, heap và `log_stage4.csv`.
9. Chạy `compare_stages.py` sau khi có đủ CSV để cập nhật bảng tổng hợp và biểu đồ.

**Kết quả cần giải thích:**

Stage 4 có payload khoảng 168 B, AES khoảng 491,7 µs, TLS handshake khoảng 757 ms và heap khoảng 227,9 KB. Đây là Stage có nhiều lớp bảo vệ nhất nhưng cũng có chi phí tài nguyên cao nhất.

**Câu nói khi bảo vệ:**

> AES-GCM bảo vệ payload kể cả khi payload được lưu hoặc đi qua một điểm đã kết thúc TLS; TLS bảo vệ kênh truyền. Hai cơ chế bổ sung cho nhau nhưng không loại bỏ nhu cầu quản lý khóa an toàn.

#### 3.5.1. Thứ tự chạy khi trình diễn nhanh

Nếu thời gian bảo vệ ngắn, nhóm có thể trình bày theo thứ tự sau:

```text
1. Cho xem mạch ESP32-S3–DHT11 và vị trí GPIO4.
2. Cho xem Stage 1 để chứng minh pipeline plaintext.
3. Cho xem Stage 2 để chứng minh Base64, AES-GCM và tag.
4. Cho xem Stage 3 để chứng minh cổng 8883 và TLS handshake.
5. Cho xem Stage 4 để chứng minh kết hợp AES-GCM + TLS.
6. Mở CSV và comparison_charts.png để giải thích số liệu.
7. Chạy hoặc trình bày tamper test để chứng minh dữ liệu bị sửa bị từ chối.
```

Không nên flash liên tục nhiều lần nếu thời gian bảo vệ hạn chế hoặc phần cứng không ổn định. Khi đó, dùng log và CSV đã thu được làm bằng chứng, đồng thời nói rõ đó là kết quả của lần chạy thực nghiệm đã ghi nhận.
