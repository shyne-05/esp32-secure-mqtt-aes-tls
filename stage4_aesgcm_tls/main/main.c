...
static const char *TAG = "STAGE4_AESGCM_TLS";
...
void app_main(void)
{
    ...
    ESP_LOGI(TAG, "=== STAGE 4: AES-256-GCM + TLS 1.3 (2-layer encryption) ===");
    ...
    while (1) {
        if (mqtt_connected) {
            ...
            snprintf(draft_plain, sizeof(draft_plain),
                     "{\"id\":%lu,\"temp\":%.1f,\"humid\":%.1f,\"ts\":%lld,\"heap\":%lu,\"enc_us\":0,\"tls_ms\":%lu,\"cpu_pct\":0.0}",
                     msg_count, temp, humid, ts, heap_at_send, tls_handshake_ms);
            ...
            // Prepare the actual plaintext JSON with the estimated enc_us and tls_ms
            char plaintext[MAX_PLAIN_LEN];
            snprintf(plaintext, sizeof(plaintext),
                     "{\"id\":%lu,\"temp\":%.1f,\"humid\":%.1f,\"ts\":%lld,\"heap\":%lu,\"enc_us\":%lld,\"tls_ms\":%lu,\"cpu_pct\":0.0}",
                     msg_count, temp, humid, ts, heap_at_send, enc_us_estimate, tls_handshake_ms);
            ...
            // Put computed cpu_pct directly in the plaintext and encrypt it.
            snprintf(plaintext, sizeof(plaintext),
                     "{\"id\":%lu,\"temp\":%.1f,\"humid\":%.1f,\"ts\":%lld,\"heap\":%lu,\"enc_us\":%lld,\"tls_ms\":%lu,\"cpu_pct\":%.1f}",
                     msg_count, temp, humid, ts, heap_at_send, enc_time, tls_handshake_ms, cpu_pct);
            
            enc_len = encrypt_and_encode(plaintext, payload, sizeof(payload));
            if (enc_len > 0) {
                payload[enc_len] = '\0';
                int msg_id = esp_mqtt_client_publish(mqtt_client, "dothi/sensor/aesgcm_tls", payload, 0, 1, 0);
                ESP_LOGI(TAG, "Sent Msg #%lu (Encrypted + TLS): Plaintext=%s (msg_id: %d, enc_time: %lld us, tls_ms: %lu, cpu_pct: %.3f)",
                         msg_count, plaintext, msg_id, enc_time, tls_handshake_ms, cpu_pct);
                msg_count++;
            }
        } else {
